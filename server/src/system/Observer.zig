...
pub fn init(position: nz.Vec3(f32)) Observer {
    return .{ .position = position, .yaw_rotation = .identity, .pitch = 0, .speed = 20, .follow = null };
}

pub fn rotation(self: Observer) Quat {
...
pub fn update(self: *Observer, window: *Window, delta_time: f32, player_count: usize) void {
