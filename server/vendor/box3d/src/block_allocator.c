...
b3BlockAllocator b3CreateBlockAllocator( int elementSize, int initialCount )
{
	...
	// PlanetaryZigma patch: round element size up to 8 so the freelist void* store
	// in b3FreeElement stays 8-aligned (b3Alloc gives a 64-aligned base). Upstream
	// leaves odd slots 4-aligned — a misaligned pointer store that is UB per the C
	// standard and aborts under UBSan. Re-apply on box3d upgrade. See [[project-box3d-port]].
